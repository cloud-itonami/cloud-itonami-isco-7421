(ns electronicsmech.actor
  "ElectronicsMechActor — the ISCO-08 7421 electronics-repair service
  scheduling/logistics coordination actor as a
  `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one service-coordination operation request
  ...
  resume after human sign-off. Modeled on cloud-itonami-isco-7127's
  hvacmech.actor (closest domain shape, itself modeled on
  cloud-itonami-isco-7111's housebuilder.actor and
  cloud-itonami-isco-3313's accountingsupport.actor /
  cloud-itonami-isco-9311's mininglabor.actor for the
  ...
  The unconditional invariant: the Electronics Mechanic Advisor can
  never directly commit an operating record the ElectronicsMechGovernor
  refuses, and can never make this actor finalize an
  electronics-repair-execution decision (e.g. proceeding with a
  specific electronics repair) or override/bypass a
  shop-safety-officer's judgment — every commit-record! call is gated
  behind `:decide`, and that decision class is a permanent hard block
  regardless of disposition path. This actor coordinates SERVICE
  SCHEDULING/LOGISTICS ONLY — it never performs electronics-repair work
  or authorizes electronics-repair execution itself."
...
(defn build-graph
  "Build a compiled ElectronicsMechActor graph. `store` implements
  `electronicsmech.store/Store`. `advisor` implements
  `electronicsmech.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:technician-id (:technician-id request)
                                    :op (:op proposal)
                                    :service-account-id (:service-account-id proposal)
                                    :payload proposal}]
